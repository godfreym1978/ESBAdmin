...
import com.ibm.datapower.wamt.clientAPI.NotManagedException;
import com.ibm.datapower.wamt.clientAPI.MacroProgressContainer;
import com.ibm.mq.constants.MQConstants;

public class DPUtil {

	public void registerDevice(String symbolicName,String hostName,String userID, String password, int hlmPort){
		try{
	    	//Get an instance of the manager. All subsequent calls to getInstance will return the same 
	        ...
	        //Create device object, note the use of a ProgressContainer since the amount of time needed
	        //to communicate with the device is indeterminate.
	        System.out.println("Create Device: device1");                   
	        progressContainer = null; 
	        progressContainer = Device.createDevice(symbolicName, hostName, userID, password, hlmPort);
            ...
            int deviceCtr = 0;
                ...
                while (domainCtr<sc.size()){
                	//System.out.println(sc.get(domainCtr).toString() + " is to be added now");
	...
	//public void getServices(String symbolicName,String hostName,String userID, String password, int hlmPort){
			...
			try{
	            ...
	            while(deviceCtr < devices.length){
	                ...
	                while (domainCtr<sc.size()){
	                	...
	                	RuntimeService rs[] = devices[deviceCtr].getManagedDomain(sc.get(domainCtr)).getServices(); 
	                	...
	                	while (serviceCtr < rs.length){
	                		...
	                		ReferencedObjectCollection refObjColl = rs[serviceCtr].getReferencedObjectsAndFiles();
	                		
	                		try{
		                		StringCollection refFiles = refObjColl.getReferencedFiles();
		                		int refFileCtr = 0;
		                		while(refFileCtr<refFiles.size()){
		                			fileListDtl.add(refFiles.get(refFileCtr).toString());
		                			refFileCtr++;
		                		}
	                		...
	                		try{
		                		com.ibm.datapower.wamt.amp.ConfigObject[] configObj = refObjColl.getReferencedObjects();
		                		int configObjCtr = 0;
		                		while(configObjCtr<configObj.length){
		                			configListDtl.add(configObj[configObjCtr].getName());
	                		...
	                		if(rs[serviceCtr].getClassName().equals("MultiProtocolGateway")){
		                		mpgListDtl.add(iMap);
